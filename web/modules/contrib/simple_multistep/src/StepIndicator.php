...
class StepIndicator extends FormStep {
   ...
   *   Current step.
   */
  public function __construct(array $form, FormStateInterface $form_state, int $current_step) {
   ...
   */
  private function createIndicator(): array {
    $steps_label = [
      '#type' => 'item',
      '#weight' => -1,
    ];
    ...
    foreach ($this->steps as $step_number => $step) {
      ...
      if ($format_settings['show_step_title']) {
        ...
        $markup .= '<div id="step-label-' . $step_number . '" class="step-label' . $active . '">';
        $markup .= $step->label;
        $markup .= '</div>';
      }
    }
